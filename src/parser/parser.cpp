...
nodePtrType Parser::create_processing(){
    ...
    tokenType token(tem_name+create_object, "statement");
...
nodePtrVecType Parser::create_table_processing(){
    ...
    tokenType table_name_token(get_token().first, "table_name");
    next_token();
...
nodePtrType Parser::col_def_processing(){
    ...
    std::cout << get_token().first << std::endl;
    std::cout << get_token().second << std::endl;
    if (iter == iter_end){
        std::cout << "error" << std::endl;
        exit(1);
    } else if (get_token().second != "identifier"){
        std::cout << "col_name must be identifier!" << std::endl;
        exit(1);
    }
